...
public class ProjectController {
     ...
     */
    @RequestMapping(value = Constants.MODIFY_PROJECT, method = RequestMethod.POST)
    public ModelAndView getProjectToModify(@RequestParam(Constants.ID_COLUMN)
        ...
        try {
            ...
            Logger.error(e);
            return new ModelAndView(Constants.ERROR_JSP, Constants.ERROR,
                Constants.PROJECT_SEARCH_FAILED);
        }
    }
    ...
    public ModelAndView assignEmployees(@RequestParam(value=Constants.
            EMPLOYE,required=false) String[] ids, @RequestParam(Constants.
            ID_COLUMN) Integer projectId) {

        try {
            projectService.assignEmployeeToProject(projectId, ids);
            Project project = projectService.searchProjectById(projectId);
    ...
    public ModelAndView removeEmployeeFromProject(Model model, @RequestParam(
        ...
        try {
            projectService.removeEmployeeFromProject(projectId, ids);
            Project project = projectService.searchProjectById(projectId);
            return new ModelAndView(Constants.SEARCH_PROJECT_JSP, Constants.
                PROJECT, project);
        } catch (ApplicationException e) {
     ...
     */
    @RequestMapping(value=Constants.ACTIVATE_PROJECT, method = RequestMethod.POST)
    private ModelAndView activateProject(@RequestParam(Constants.ID_COLUMN)
            Integer projectId) {
        try {
            ...
            Logger.error(e);
            return new ModelAndView(Constants.ERROR_JSP, Constants.ERROR,
                Constants.PROJECT_ACTIVATE_FAIL);
        }
    }
}
